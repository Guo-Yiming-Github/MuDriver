...
while isempty(strfind(linea,'# Begin: Data Text'))==1
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%This one is for simulation time:
    if isempty(strfind(linea,'# Desc: Total simulation time:'))~=1        
        remain = linea;
        while true
            [str, remain] = strtok(remain);
            if strcmp(str,'time:')==1
                [str, remain] = strtok(remain);
                data.time =  str2num(str);
            end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%
    if isempty(strfind(linea,'# xstepsize:'))~=1        
        remain = linea;        
        [~, remain] = strtok(remain);
        [~, remain] = strtok(remain);
        [str, ~] = strtok(remain);
        data.xstepsize=  str2num(str);        
    end    
    %%%%%%%%%%%%%%%%%%    
    
    %%%%%%%%%%%%%%%%%%%%%%%%
    if isempty(strfind(linea,'# ystepsize:'))~=1        
        remain = linea;        
        [~, remain] = strtok(remain);
        [~, remain] = strtok(remain);
        [str, ~] = strtok(remain);
        data.ystepsize=  str2num(str);        
    end    
    %%%%%%%%%%%%%%%%%%    
    
    %%%%%%%%%%%%%%%%%%%%%%%%
    if isempty(strfind(linea,'# zstepsize:'))~=1        
        remain = linea;        
        [~, remain] = strtok(remain);
        [~, remain] = strtok(remain);
        [str, ~] = strtok(remain);
        data.zstepsize=  str2num(str);        
    end    
    %%%%%%%%%%%%%%%%%%    
...
x_step = data.xmax/data.xnodes;
y_step = data.ymax/data.ynodes;
z_step = data.zmax/data.znodes;
x=linspace(data.xmin+x_step/2,data.xmax-x_step/2,data.xnodes);
y=linspace(data.ymin+y_step/2,data.ymax-y_step/2,data.ynodes);
z=linspace(data.zmin+z_step/2,data.zmax-z_step/2,data.znodes);
